...
exports.createUser = function createUser(req, res, next) {
  ...
  workflow.on('validateUser', function validateUser() {
    ...
    req.checkBody('permissions').notEmpty().withMessage('Permissions should not be Empty').withMessage('Invalid User type');
            ...
            config.MySQL.query(check_sql, [body.email], function (err, results) {
              ...
              switch(results.length)
              {
                case 1:
                  ...
                  break
                case 0:
                  var sql = "INSERT INTO "+tbl+" (email, password, permissions) VALUES (?,?,?)";
                  var Permitted = [body.email, hash, body.permissions];
